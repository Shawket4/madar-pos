#!/usr/bin/env bash
# Generates the shared Madar icon set (Lucide — the clean, MIT, React/shadcn
...
#   - swift-app/Sources/MadarUI/Theme/IconCatalog.swift   (SF name -> asset name)
...
SWIFT_CAT="$ROOT/swift-app/Sources/MadarUI/Theme/IconCatalog.swift"
KT_CAT="$ROOT/kotlin-app/composeApp/src/commonMain/kotlin/app/madar/ui/IconCatalog.kt"
...
# ---- Swift catalog: SF name -> asset (lucide) name ----
{
  ...
  echo "let madarIconAsset: [String: String] = ["
...
# ---- Kotlin catalog: SF name -> DrawableResource ----
{
  ...
  echo "package app.madar.ui"
  echo ""
  echo "import org.jetbrains.compose.resources.DrawableResource"
  echo "import app.madar.resources.Res"
  # one import per unique drawable
  for lucide in $UNIQUE_LUCIDE; do
    echo "import app.madar.resources.ic_${lucide//-/_}"
  done
  echo ""
