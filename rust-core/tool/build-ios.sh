...
# assembles SufrixCore.xcframework consumed by swift-app.
#
# Output: rust-core/target/SufrixCore.xcframework  +  bindings/swift/*
set -euo pipefail
...
cargo build $BUILD_FLAG -p sufrix-core --target "$DEVICE"
cargo build $BUILD_FLAG -p sufrix-core --target "$SIM_ARM"
cargo build $BUILD_FLAG -p sufrix-core --target "$SIM_X86"

OUT="$CORE_DIR/target"
SIM_UNI="$OUT/ios-sim-universal"
mkdir -p "$SIM_UNI"
lipo -create \
  "$OUT/$SIM_ARM/$PROFILE/libsufrix_core.a" \
  "$OUT/$SIM_X86/$PROFILE/libsufrix_core.a" \
  -output "$SIM_UNI/libsufrix_core.a"

echo "── Generating Swift bindings + modulemap…"
GEN="$OUT/swift-gen"
rm -rf "$GEN" && mkdir -p "$GEN"
cargo run $BUILD_FLAG -p sufrix-core --bin uniffi-bindgen -- generate \
  --library "$OUT/$DEVICE/$PROFILE/libsufrix_core.a" \
...
rm -rf "$OUT/SufrixCore.xcframework"
xcodebuild -create-xcframework \
  -library "$OUT/$DEVICE/$PROFILE/libsufrix_core.a" -headers "$HEADERS" \
  -library "$SIM_UNI/libsufrix_core.a" -headers "$HEADERS" \
  -output "$OUT/SufrixCore.xcframework"

echo "Done: $OUT/SufrixCore.xcframework"
