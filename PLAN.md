...
│   ├── tool/
│   │   └── generate_api.sh          # regenerates crates/madar-api from MadarRust/openapi.json
│   └── crates/
...
- `tool/generate_api.sh` runs `openapi-generator -g rust` against `MadarRust/openapi.json` into `crates/madar-api` (excluded from the workspace; first run is *expected* not to fully compile — see §10/D2).
...
Backend = `MadarRust` (Actix-Web). Everything is **additive**; no field removed, no existing status code changed. (Several pieces already exist — reuse, don't rebuild.)
