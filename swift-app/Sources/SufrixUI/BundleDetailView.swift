...
struct BundleDetailView: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: Space.sm) {
            VStack(alignment: .leading, spacing: 2) {
                Text(bundle.name).font(.ui(18, .heavy)).foregroundStyle(theme.colors.textPrimary)
    ...
    private func statusIcon(configurable: Bool, configured: Bool) -> some View {
        ...
        return Image(systemName: symbol).font(.system(size: 16)).foregroundStyle(color).frame(width: 22)
    }
