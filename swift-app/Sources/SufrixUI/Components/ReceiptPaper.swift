...
struct ReceiptPaper: View {
    ...
    private var header: some View {
        VStack(spacing: 3) {
            ...
            Text(storeName.isEmpty ? "SUFRIX" : storeName.uppercased())
                .font(.system(size: 15, weight: .bold, design: .monospaced))
            if receipt.isDelivery, let ch = receipt.deliveryChannel {
                Text("— \(ch == "in_mall" ? "IN-MALL" : "DELIVERY") —")
                    .font(.system(size: 11, design: .monospaced)).foregroundStyle(faint)
            }
        }
    ...
    private var meta: some View {
        VStack(spacing: 2) {
            row(orderTitle, Self.dateTime(receipt.createdAt))
    ...
    private func row(_ left: String, _ right: String, bold: Bool = false, faint isFaint: Bool = false) -> some View {
        HStack(alignment: .top, spacing: 6) {
            Text(left)
                .font(.system(size: bold ? 13 : 12, weight: bold ? .bold : .regular, design: .monospaced))
            Spacer(minLength: 4)
            if !right.isEmpty {
                Text(right)
                    .font(.system(size: bold ? 13 : 12, weight: bold ? .bold : .regular, design: .monospaced))
    ...
    static func dateTime(_ rfc3339: String) -> String {
        ...
        let date = iso.date(from: rfc3339)
            ?? { let f = ISO8601DateFormatter(); f.formatOptions = [.withInternetDateTime, .withFractionalSeconds]; return f.date(from: rfc3339) }()
        guard let date else { return rfc3339 }
        let out = DateFormatter()
        out.dateFormat = "dd/MM/yyyy hh:mm a"
