...
struct MenuItemCard: View {
    ...
    var body: some View {
        Button {
            ...
            // Lock every card to ONE shape → uniform heights → even grid rows,
            // regardless of the image loading or the name wrapping (Flutter's
            // GridView childAspectRatio). The hero fills whatever's above the footer.
    ...
    private var hero: some View {
        ...
        // Fills the space the card gives it (card height − footer); the card's
        // aspectRatio governs the overall shape now.
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .clipped()
    }
...
/// Map a core `CatStyleView.icon` key to a SHARED Lucide icon name (resolves via
/// the icon catalog). Returns nil for the `cafe` default — i.e. an unrecognized /
/// custom category — so the caller falls back to the category monogram instead of
/// a wrong coffee cup. Categories are free-form (user-created with an optional
/// uploaded image), so the order is: uploaded image → this icon → monogram.
func categoryIconName(_ key: String) -> String? {
    switch key {
    case "coffee", "mocha", "tea", "bakery", "lunch", "icecream", "drink", "water", "ice", "matcha":
        return "cat.\(key)"
    default:
        return nil // "cafe" (the core's unmatched default) + anything else → monogram
    }
}

/// Up to two initials for a category name (same rule as the item monogram).
func categoryMonogram(_ name: String) -> String {
